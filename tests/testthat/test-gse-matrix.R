...
testthat::test_that("empty GSE is handled correctly", {
    gse <- skip_if_download_fail(
        geo_matrix("GSE11413", odir = tempdir(), add_gpl = TRUE)
    )
...
testthat::test_that("case-mismatched IDs in GSEs handled correctly", {
    gse <- skip_if_download_fail(geo_matrix("GSE35683", odir = tempdir()))
    testthat::expect_equal(nrow(gse), 54675L, ignore_attr = TRUE)
})

testthat::test_that("single-sample GSE handled correctly", {
    gse <- skip_if_download_fail(geo_matrix("GSE11595", odir = tempdir()))
...
testthat::test_that("short GSE handled correctly", {
    gse <- skip_if_download_fail(geo_matrix("GSE34145", odir = tempdir()))
...
testthat::test_that("GSE with more than one value per characteristic handled", {
    gse <- skip_if_download_fail(geo_matrix("GSE71989", odir = tempdir()))
    testthat::expect_equal(nrow(gse), 54675L, ignore_attr = TRUE)
    testthat::expect_equal(ncol(gse), 22L, ignore_attr = TRUE)
})

testthat::test_that("GSE has populated experimentData", {
    gse <- skip_if_download_fail(geo_matrix("GSE53986", odir = tempdir()))
...
testthat::test_that("GSE populates experimentData as much as possible", {
    gse <- skip_if_download_fail(geo_matrix("GSE99709", odir = tempdir()))
    ed <- Biobase::experimentData(gse)
    ...
    # ----------------------------------------------------------------
    gse <- skip_if_download_fail(geo_matrix("GSE27712", odir = tempdir()))
...
testthat::test_that("GSE/GPL with integer64 columns handled correctly", {
    gse <- skip_if_download_fail(geo_matrix("GSE7864", odir = tempdir(), add_gpl = TRUE))
...
testthat::test_that("GSE/GPL with Bioconductor annotation package handled correctly", {
    gse <- skip_if_download_fail(geo_matrix("GSE410", odir = tempdir(), add_gpl = NULL))
...
testthat::test_that("non-existed GSE matrix files signal error", {
    testthat::expect_error(
        skip_if_download_fail(suppressWarnings(geo_matrix("gse1787", odir = tempdir()))),
        regexp = "Failed to open.*GSE1787"
    )
})

testthat::test_that("GSE/GSM with characteristic column worked well", {
    gse <- skip_if_download_fail(geo_matrix(
        "GSE53987",
        odir = tempdir(),
